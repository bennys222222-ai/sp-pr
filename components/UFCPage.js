        ...
        {showInsights && (
          <div className="insights-popup" ref={insightsRef}>
            <div className="insights-header">
              {/* 🔥 HOT STREAK BANNER */}
              {(() => {
                const analyzeFighterStreak = (fightHistory) => {
                ...
                const streak1 = analyzeFighterStreak(left?.fightHistory);
                const streak2 = analyzeFighterStreak(right?.fightHistory);
                const badge1 = getStreakBadge(streak1);
                const badge2 = getStreakBadge(streak2);

                if (!badge1 && !badge2) return null;

                return (
                  <div className="hot-streak-banner">
                    {badge1 && (
                      <div className={`streak-badge ${badge1.class}`} style={{ borderColor: badge1.color }}>
                        <span className="streak-icon">🔥</span>
                        <span className="streak-text">{badge1.text}</span>
                      </div>
                    )}
                    {badge2 && (
                      <div className={`streak-badge ${badge2.class}`} style={{ borderColor: badge2.color }}>
                        <span className="streak-icon">🔥</span>
                        <span className="streak-text">{badge2.text}</span>
                      </div>
                    )}
                  </div>
                );
                  ...
                  <div className="insights-section">
                    ...
                    <div className="stats-grid">
                      ...
                      <div className="stat-row">
                        ...
                        <span className="stat-value stat-value--left">{left.advancedMetrics.dangerZones.ground}</span>
                        <span className="stat-vs">vs</span>
                        <span className="stat-value stat-value--right">{right.advancedMetrics.dangerZones.ground}</span>
                      </div>
                    </div>
                  </div>
                )}
